...
import { X, Loader2, Wrench, Package, AlertTriangle, CheckCircle2, Minus } from "lucide-react";
...
export default function AdHocProduceDrawer({ style, hasBom, onClose, onEditBom, onDone }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full sm:max-w-4xl border-2 border-slate-900 shadow-ind-lg flex flex-col max-h-[100dvh] sm:max-h-[92vh]"
        onClick={(e) => e.stopPropagation()}
        data-testid="adhoc-produce-drawer"
      >
        {/* ── Header (sticky) ── */}
        <div className="px-4 sm:px-5 py-4 border-b-2 border-slate-900 bg-slate-50 flex items-center justify-between gap-3 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-slate-200 rounded flex-shrink-0 touch-manipulation"
            data-testid="adhoc-close"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          {results ? (
            <div className="p-4 sm:p-5 space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <BtnSecondary onClick={() => { setResults(null); setQty({}); }} className="flex-1 min-h-[44px]">Produce more</BtnSecondary>
                <BtnPrimary onClick={onDone} className="flex-1 min-h-[44px]" data-testid="adhoc-done">Done</BtnPrimary>
              </div>
            </div>
          ) : (
            <div className="p-4 sm:p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] uppercase tracking-wider font-bold text-slate-600 block mb-1">Add color</label>
                  {/* Existing chips */}
                  {colors.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {colors.map((c) => (
                        <span key={c} className="inline-flex items-center gap-1 bg-slate-100 border border-slate-300 px-2 py-1 text-xs font-mono font-bold">
                          {c}
                          <button
                            onClick={() => removeColor(c)}
                            className="ml-0.5 p-1 text-slate-400 hover:text-red-600 touch-manipulation min-w-[24px] min-h-[24px] flex items-center justify-center"
                  ...
                  <div className="flex gap-1 mt-0.5">
                    <input
                      ...
                      placeholder="e.g. Tan"
                      className="flex-1 border-2 border-slate-300 px-3 py-3 text-sm min-h-[44px]"
                      data-testid="adhoc-new-color"
                    />
                    <BtnSecondary onClick={addColor} className="min-h-[44px]" data-testid="adhoc-add-color">Add</BtnSecondary>
                  </div>
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-wider font-bold text-slate-600 block mb-1">Add size</label>
                  {/* Existing chips */}
                  {sizes.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {sizes.map((s) => (
                        <span key={s} className="inline-flex items-center gap-1 bg-slate-100 border border-slate-300 px-2 py-1 text-xs font-mono font-bold">
                          {s}
                          <button
                  ...
                  <div className="flex gap-1 mt-0.5">
                    <input
                      ...
                      placeholder="e.g. 7"
                      className="flex-1 border-2 border-slate-300 px-3 py-3 text-sm font-mono min-h-[44px]"
                      inputMode="numeric"
                      data-testid="adhoc-new-size"
                    />
                    <BtnSecondary onClick={addSize} className="min-h-[44px]" data-testid="adhoc-add-size">Add</BtnSecondary>
                  </div>
                </div>
              </div>
              ...
              {colors.length > 0 && sizes.length > 0 && (
                <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
                  <table className="text-xs border-collapse">
                    <thead>
                      <tr>
                        <th className="border border-slate-300 bg-slate-100 px-2 py-2 sticky left-0 z-10 text-left">Color \ Size</th>
                        {sizes.map((s) => (
                          <th key={s} className="border border-slate-300 bg-slate-50 px-1 py-2 font-mono min-w-[52px] text-center">
                            {s}
                          </th>
                        ))}
                        <th className="border border-slate-300 bg-slate-200 px-2 py-2">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {colors.map((c) => (
                        <tr key={c}>
                          <th className="border border-slate-300 bg-slate-50 px-2 py-1 text-left sticky left-0 z-10 font-mono whitespace-nowrap">
                            {c}
                          </th>
                          {sizes.map((s) => (
                            <td key={s} className="border border-slate-300 p-0">
                              <input
                                ...
                                className={`w-full min-w-[52px] text-center font-mono py-2.5 focus:outline-none focus:bg-emerald-50 min-h-[44px] ${cellVal(c, s) > 0 ? "bg-emerald-50 font-bold" : "bg-white"}`}
                                inputMode="numeric"
                                data-testid={`matrix-cell-${c}-${s}`}
                              />
                            </td>
                          ))}
                          <td className="border border-slate-300 bg-slate-100 px-2 py-2.5 text-center font-mono font-bold">{rowTotal(c) || "·"}</td>
                        </tr>
                      ))}
                      <tr>
                        <th className="border border-slate-300 bg-slate-200 px-2 py-2 text-left sticky left-0 z-10 uppercase text-[10px]">Total</th>
                        {sizes.map((s) => (
                          <td key={s} className="border border-slate-300 bg-slate-100 px-1 py-2 text-center font-mono font-bold">{colTotal(s) || "·"}</td>
                        ))}
                        <td className="border border-slate-300 bg-slate-900 text-white px-2 py-2 text-center font-mono font-black text-base" data-testid="matrix-grand-total">{grandTotal}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              {/* ── BOM deduction toggle ── */}
              <label className="flex items-start gap-3 cursor-pointer text-xs pt-1 touch-manipulation">
                <input
                  type="checkbox"
                  checked={useComponents}
                  onChange={(e) => setUseComp(e.target.checked)}
                  className="w-5 h-5 mt-0.5 flex-shrink-0 accent-[#C27842]"
                  data-testid="adhoc-use-components"
                />
              ...
              <div className="flex gap-2 pt-1">
                <BtnSecondary onClick={onEditBom} className="flex-1 min-h-[44px]">
                  <Wrench className="w-3.5 h-3.5 inline mr-1" />Edit Production Card
                </BtnSecondary>
                <BtnPrimary onClick={submit} disabled={busy || grandTotal <= 0} className="flex-1 min-h-[44px]" data-testid="adhoc-submit">
            ...
            <div className="p-5 space-y-3 text-sm">
              ...
              <div className="border-2 border-red-200 divide-y divide-red-200">
                {shortageConfirm.shortages.map((s, i) => (
                  <div key={i} className="px-3 py-2.5 flex items-baseline justify-between gap-2 text-xs min-h-[44px]">
                    <div>
              ...
              <div className="flex gap-2">
                <BtnSecondary onClick={shortageConfirm.onCancel} className="flex-1 min-h-[44px]" data-testid="shortage-cancel">Cancel</BtnSecondary>
                <BtnPrimary onClick={shortageConfirm.onConfirm} className="flex-1 min-h-[44px] !bg-red-700 !border-red-700 hover:!bg-red-800" data-testid="shortage-proceed">
                  Proceed anyway
                </BtnPrimary>
              </div>
            </div>
